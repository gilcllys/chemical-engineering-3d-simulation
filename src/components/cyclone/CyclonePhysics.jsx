  ...
  useFrame((_, delta) => {
    ...
    for (let s = 0; s < SUB; s++) {
      for (const p of pool.current) {
        ...
        // ── FASE: Queda na caixa coletora — padrão Example 2 (areia) ──
        } else if (p.phase === 'underflow_fall') {
          if (p.settled) continue

          if (fillLevel.current === null) fillLevel.current = BOX_FLOOR

          // Gravidade: F = m·g escalada visualmente (Example 2 pattern: v += g·dt)
          const GRAVITY = 12.0
          p.vy = (p.vy || 0) - GRAVITY * sdt

          // Arrasto do ar global por frame (Example 2: v *= 1 - drag)
          const AIR_DRAG = 0.012
          p.vx = (p.vx || 0) * (1 - AIR_DRAG)
          p.vz = (p.vz || 0) * (1 - AIR_DRAG)
          ...
          // Colisão com o piso / nível do pó (Example 2 padrão exato adaptado p/ areia)
          if (p.y < effectiveFloor + p.sphereR) {
            p.y = effectiveFloor + p.sphereR

            const SAND_BOUNCE    = 0.07   // restituição muito baixa (areia)
            const FLOOR_FRICTION = 0.18   // fricção no impacto

            p.vy = -p.vy * SAND_BOUNCE
            p.vx *= FLOOR_FRICTION
            p.vz *= FLOOR_FRICTION

            // Ângulo de repouso: quando a pilha cresce, empurra radialmente
            const pileRatio = Math.max(0, (fillLevel.current - BOX_FLOOR) / FILL_HEIGHT)
            if (pileRatio > 0.02) {
              const r = Math.sqrt((p.x || 0) ** 2 + (p.z || 0) ** 2)
              const pushStrength = pileRatio * 1.5
              if (r > 0.06) {
                p.vx += (p.x / r) * pushStrength * FLOOR_FRICTION
                p.vz += (p.z / r) * pushStrength * FLOOR_FRICTION
              } else {
                const ang = Math.random() * Math.PI * 2
                p.vx += Math.cos(ang) * pushStrength * FLOOR_FRICTION
                p.vz += Math.sin(ang) * pushStrength * FLOOR_FRICTION
              }
            }

            // Assentamento: velocidade total baixa após ricochete → settle
            const totalSpeed = Math.sqrt((p.vx || 0) ** 2 + (p.vy || 0) ** 2 + (p.vz || 0) ** 2)
            if (totalSpeed < 0.05) {
              p.vx = 0; p.vy = 0; p.vz = 0
              p.settled = true
              p.y = effectiveFloor + p.sphereR * (0.85 + Math.random() * 0.3)
              fillLevel.current = Math.min(BOX_TOP - 0.02, fillLevel.current + FILL_PER_PART)
            }
          }

          // Paredes da caixa (baixa restituição, tipo areia batendo na parede)
