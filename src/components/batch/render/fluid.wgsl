// binding(0) reserved — sampler removed (textureLoad does not require a sampler;
//   declaring an unused sampler causes it to be omitted from the auto-layout
//   while still appearing in the bind group, which triggers a validation error)
...
fn fs(input: FragmentInput) -> @location(0) vec4f {
    ...
    let bgColor: vec3f = vec3f(0.86, 0.93, 1.0);
    ...
    let skyTop = vec3f(0.70, 0.88, 1.00);
    let skyBot = vec3f(0.10, 0.35, 0.65);
    let skyT = clamp(reflectionDirWorld.y * 0.5 + 0.5, 0.0, 1.0);
    var reflectionColor: vec3f = mix(skyBot, skyTop, skyT);
